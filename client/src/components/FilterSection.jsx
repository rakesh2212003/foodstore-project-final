...
export const FilterCard = ({ index, data, category, setCategory }) => {
    return (
        <motion.div
            key={index}
            ...
            className={`
                group w-28 min-w-[120px] cursor-pointer rounded-md py-2
                ${category === data.category
                    ? 'bg-orange-500'
                    : 'bg-primary'
                }
                hover:bg-orange-500 shadow-md flex flex-col items-center justify-center gap-2
            `}
        >
            <div className={`
                w-10 h-10 rounded-full shadow-md flex items-center justify-center group-hover:bg-primary
                ${category === data.category
                    ? 'bg-primary'
                    : 'bg-orange-500'
                }
